...
const Categories:React.FC<any> = ({ pagePermission })=> {
  ...
  useEffect(() => {
    console.log("listing data updated")
  }, [listingData]);
  ...
  const generateTbody = (value:any, index:any) => {
    return [
      {
        name: "sr_no",
        value: index + 1,
  ...
  const changeFilterFieldValue = (field:any, value:any, currentListing = []) => {
    if (field.includes("filter_parent_category")) {
      setReRender(!reRender)
      var arr = currentListing;
      arr.forEach((element:any) => {
        if (element.filters[0].name === field) {
          fetch(URL + "api/Categories/" + (value === "-1" ? "GetCategoriesByLevel?level=" + Number(field.replace("filter_parent_category", "")) : "GetCategoriesByParentId?id=" + value), {
            method: "GET",
            headers: {
              Authorization:
                "Bearer " +
      ...
      if (obj.defaultValue.label === "All") {
        ...
        allListingData.forEach((element:any) => {
  ...
  const fetchDataForEdit = (city_id:any) => {
    // setUpdateMode(false);
